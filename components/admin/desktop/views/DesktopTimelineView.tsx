...
export interface DesktopTimelineViewProps {
  ...
  /** Needed so «Оновити» can edit existing Telegram messages without a slow digest. */
...
export function DesktopTimelineView({
  ...
  const refreshTelegramTurnovers = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/admin/telegram/refresh-turnovers", {
        method: "POST",
        headers: {
      ...
      const edited = Number(data.edited) || 0;
      const sent = Number(data.sent) || 0;
      const deleted = Number(data.deleted) || 0;
      showToast(
        edited + sent + deleted > 0
          ? `Telegram оновлено: ${edited} змінено, ${sent} надіслано${
              deleted ? `, ${deleted} видалено` : ""
            }`
          : "Немає змін у Telegram (перевірте заїзд/виїзд на сьогодні)"
      );
    } catch (e) {
  ...
  const telegramRefreshButton = (
    <button
      ...
      title="Оновити сповіщення заїзд/виїзд і прибирання в Telegram"
      style={
        isMobile
          ? {
              height: 40,
    ...
    >
      <RefreshCw
        size={isMobile ? 14 : 15}
        strokeWidth={2.5}
        style={telegramRefreshBusy ? { animation: "timeline-undo-spin 0.8s linear infinite" } : undefined}
        aria-hidden
      />
      {!telegramRefreshBusy && "Оновити"}
    </button>
  );
